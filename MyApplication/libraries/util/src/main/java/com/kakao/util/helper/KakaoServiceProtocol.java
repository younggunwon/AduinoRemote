...
/**
 * Talk을 비롯하여 카카오 서비스와 통신하기 위한 protocol
 * authorization을 각 서비스를 통해 하는 경우
 * [Notice] talk이 젤 먼저 적용되면서 key이름에 모두 talk이 붙어 있는데 실제로는 모든 카카오 서비스가 같은 프로토콜을 사용한다.
 *
 * @author MJ
 */
public class KakaoServiceProtocol {
    ...
    // error types
    public static final String NOT_SUPPORT_ERROR = "NotSupportError"; // KakaoTalk installed but not signed up
    public static final String UNKNOWN_ERROR = "UnknownError"; // No redirect url
    public static final String PROTOCOL_ERROR = "ProtocolError"; // Wrong parameters provided
    public static final String APPLICATION_ERROR = "ApplicationError"; // Empty redirect url
    public static final String AUTH_CODE_ERROR = "AuthCodeError";
    public static final String CLIENT_INFO_ERROR = "ClientInfoError"; // Could not fetch app info
    ...
    protected static Intent checkSupportedService(final Context context, final Intent intent, final int minVersion) {
        if (intent == null) {
            return null;
        }
    ...
    protected static Intent createIntent(final String action, final String appKey, final String redirectURI, final Bundle extras) {
    ...
    private static boolean validateTalkSignatureAndMinVersion(final Context context, final String packageName, final int minVersion) {
        ...
        for (Signature signature : talkPackageInfo.signatures) {
            ...
            if (signatureCharsString.equals(RELEASE_SIGNATURE) ||
                signatureCharsString.equals(DEBUG_SIGNATURE)) {
                return true;
            }
        }
        return false;
    ...
    public static boolean isCapriProtocolMatched(final Intent data) {
