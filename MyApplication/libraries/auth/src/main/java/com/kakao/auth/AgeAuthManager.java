...
public class AgeAuthManager {
    ...
    public static AgeAuthManager getInstance() {
        if (instance == null) {
            instance = new AgeAuthManager(new Handler(Looper.getMainLooper()), KakaoCookieManager.getInstance());
        }
        return instance;
    }

    public AgeAuthManager(final Handler handler, final KakaoCookieManager cookieManager) {
        sHandler = handler;
