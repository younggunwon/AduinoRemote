...
public class LoginButton extends FrameLayout {
    ...
    private List<AuthType> getAuthTypes() {
        ...
        if(TalkProtocol.existCapriLoginActivityInTalk(getContext(), KakaoSDK.getAdapter().getSessionConfig().getApprovalType() == ApprovalType.PROJECT)){
            availableAuthTypes.add(AuthType.KAKAO_TALK);
            availableAuthTypes.add(AuthType.KAKAO_TALK_EXCLUDE_NATIVE_LOGIN);
        }
        if(StoryProtocol.existCapriLoginActivityInStory(getContext(), KakaoSDK.getAdapter().getSessionConfig().getApprovalType() == ApprovalType.PROJECT)){
