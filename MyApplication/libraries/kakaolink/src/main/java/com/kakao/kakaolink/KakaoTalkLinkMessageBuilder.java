...
public class KakaoTalkLinkMessageBuilder {
    ...
    public KakaoTalkLinkMessageBuilder addExtra(final String key, final String value) throws KakaoParameterException {
    ...
    public KakaoTalkLinkMessageBuilder addExtra(final String key, final JSONObject value) throws KakaoParameterException {
     ...
     *
     * @param text 추가할 메시지
     * @param displayType object 의 display 여부 설정.(Permission이 없는 앱에선 동작하지 않는다.)
     * @return 카카오톡으로 전송할 하나의 메시지를 구성해주는 Builder.
     * @throws KakaoParameterException 이미 텍스트 형식의 메시지가 포함되어 있는 경우 발생한다.
     */
    public KakaoTalkLinkMessageBuilder addText(final String text, final DISPLAY_TYPE displayType) throws KakaoParameterException {
     ...
     * @throws KakaoParameterException 이미 이미지 형식의 메시지가 포함되어 있는 경우 발생한다.
     */
    public KakaoTalkLinkMessageBuilder addImage(final String src, final int width, final int height, final DISPLAY_TYPE displayType) throws KakaoParameterException {
     ...
     * 버튼 클릭시 앱연결 url에 append할 parameter가 있는 경우는 #addAppButton(String, com.kakao.kakaolink.internal.Action)을 사용한다.
     ...
     * 버튼 클릭시 kakao[appkey]://kakaolink으로 이동 하려면{@link #addAppButton(String)}를 사용한다.
     * @param text      버튼에 보일 텍스트
     ...
     * @throws KakaoParameterException 이미 버튼 형식의 메시지가 포함되어 있는 경우 발생한다.
     */
    public KakaoTalkLinkMessageBuilder addAppButton(final String text, final Action appAction, final DISPLAY_TYPE displayType) throws KakaoParameterException {
     ...
     * @throws KakaoParameterException 이미 버튼 형식의 메시지가 포함되어 있는 경우 발생한다.
     */
    public KakaoTalkLinkMessageBuilder addWebButton(final String text, final String url, final DISPLAY_TYPE displayType) throws KakaoParameterException {
        if (buttonType.getAndIncrement() == 1)
            throw new KakaoParameterException(KakaoParameterException.ERROR_CODE.DUPLICATE_OBJECTS_USED,
                "buttonType already added. each type can be added once, at most.");
     ...
     * 앱연결 url에 append할 parameter가 있는 경우 #addAppLink(String, Action, DISPLAY_TYPE) 를 사용한다.
     ...
     * @throws KakaoParameterException  이미 링크 형식의 메시지가 포함되어 있는 경우 발생한다.
     */
    public KakaoTalkLinkMessageBuilder addAppLink(final String text, final Action appAction, final DISPLAY_TYPE displayType) throws KakaoParameterException {
     ...
     * @throws KakaoParameterException 이미 링크 형식의 메시지가 포함되어 있는 경우 발생한다.
     */
    public KakaoTalkLinkMessageBuilder addWebLink(final String text, final String url, final DISPLAY_TYPE displayType) throws KakaoParameterException {
        if (linkType.getAndIncrement() == 1)
            throw new KakaoParameterException(KakaoParameterException.ERROR_CODE.DUPLICATE_OBJECTS_USED,
                "linkType already added. each type can be added once, at most.");
     ...
     * @throws KakaoParameterException 이미 링크 형식의 메시지가 포함되어 있는 경우 발생한다.
     */
    public KakaoTalkLinkMessageBuilder addInWebLink(final String text, final String url, final DISPLAY_TYPE displayType) throws KakaoParameterException {
        if (linkType.getAndIncrement() == 1)
            throw new KakaoParameterException(KakaoParameterException.ERROR_CODE.DUPLICATE_OBJECTS_USED,
                    "linkType already added. each type can be added once, at most.");
     ...
     * @param forwardable 메시지 전달 가능 여부
     * @return 카카오톡으로 전송할 하나의 메시지를 구성해주는 Builder.
     */
    ...
    public String build() throws KakaoParameterException {
        try {
            ...
            talkLinkURL.append(KakaoTalkLinkProtocol.LINKVER).append("=").append(URLEncoder.encode(KakaoTalkLinkProtocol.LINK_VERSION, KakaoTalkLinkProtocol.ENCODING)).append("&");
            ...
            talkLinkURL.append(KakaoTalkLinkProtocol.EXTRAS).append("=").append(URLEncoder.encode(extra.toString(), KakaoTalkLinkProtocol.ENCODING)).append("&");
