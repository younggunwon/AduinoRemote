...
public class KakaoLInkTestHelper {
    public static RequestConfiguration createMockRequestConfiguration() {
        AppConfig appConfig = new AppConfig("sample_app_key", "sample_key_hash", "sample_app_ver", "sample_package_name");
    ...
    public static NetworkTask createMockNetworkTask() {
        ...
        try {
            Mockito.doReturn(createMockResponseData()).when(task).request(Matchers.any(TemplateValidateRequest.class));
        } catch (IOException e) {
        }
        return task;
    }

    public static KakaoLinkSender createMockKakaoLinkSender(final KakaoLinkService linkService, final Context context) {
        KakaoLinkSender sender = Mockito.spy(new KakaoLinkSender(linkService.getTaskQueue(), linkService.getKakaoLinkApi(linkService.getNetworkTask())));
        Mockito.doReturn(sender).when(linkService).getKakaoLinkSender();
        return sender;
    }

    public static AlertDialog createMockAlertDialog(final Context context) {
        return new AlertDialog.Builder(context.getApplicationContext()).create();
    }
}
