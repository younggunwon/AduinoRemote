...
public class ConnectThread extends Thread {
    ...
    public ConnectThread(BluetoothDevice device, Handler $handler, boolean $useRead) {
        ...
        try {
            ...
            tmp = device.createRfcommSocketToServiceRecord(MainFragment.MY_UUID);

        } catch (IOException e) { }
        mmSocket = tmp;
    }

    public ConnectThread(BluetoothDevice device, Handler $handler, boolean $useRead, ArrayList<String> $filter) {
        Log.e("connectThread","make");
        Log.e("connectThread",device.getAddress());
        // Use a temporary object that is later assigned to mmSocket,
        // because mmSocket is final
    ...
    public void run() {
        ...
        ConnectedThread connectedThread = new ConnectedThread(mmSocket, _handler, useRead, filter);
